...
- **Per-model variant toggles**: Max Mode, Fast, Thinking, Caching — each with its actual pricing impact. Max Mode isn't a flat 20% upcharge. It's the upcharge *times* the provider's long-context multiplier. For Claude Opus, that's 2.4x on input. The calculator models this correctly.
